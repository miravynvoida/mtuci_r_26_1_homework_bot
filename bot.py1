...
async def disciplines_keyboard():
    ...
    cur = await conn.execute("""
        SELECT d.id, d.name, d.emoji,
               EXISTS(SELECT 1 FROM homework h WHERE h.discipline_id=d.id) AS has_hw
        FROM disciplines d
        WHERE d.active=1
...
async def discipline(c: CallbackQuery):
    ...
    cur = await conn.execute(
        "SELECT * FROM homework WHERE discipline_id=? ORDER BY due_date ASC, id DESC",
        (did,)
    )
...
async def books_main(c: CallbackQuery):
    ...
    await conn.close()
    rows = [[InlineKeyboardButton(text=f'{d["emoji"]} {d["name"]}', callback_data=f'books:{d["id"]}')] for d in ds]
    rows.append([InlineKeyboardButton(text="⬅️ Главное меню", callback_data="main")])
...
def admin_menu():
    return kb([
        [InlineKeyboardButton(text="➕ Добавить ДЗ", callback_data="adm:addhw")],
        [InlineKeyboardButton(text="📋 Управление ДЗ", callback_data="adm:hws")],
        [InlineKeyboardButton(text="📚 Дисциплины", callback_data="adm:disc")],
        [InlineKeyboardButton(text="📖 Учебники", callback_data="adm:books")],
        [InlineKeyboardButton(text="👥 Студенты", callback_data="adm:students")],
        [InlineKeyboardButton(text="📣 Уведомление всем", callback_data="adm:broadcast")],
    ])
...
async def admin_hw_delete_confirm(c: CallbackQuery):
    ...
    await replace_callback(
        ...
        kb([
            [InlineKeyboardButton(text="🗑 Да, удалить", callback_data=f"delhw:{hid}")],
...
async def delbookq(c: CallbackQuery):
    ...
    await replace_callback(c, f'🗑 Удалить учебник <b>{esc(b["title"])}</b>?', kb([
        [InlineKeyboardButton(text="🗑 Да, удалить", callback_data=f"delbook:{bid}")],
