...
async def book_back(c:CallbackQuery):
    ...
    subset=ts[p*7:p*7+7]; rows=[[b(f"📘 {x['title'][:55]}",f"book:item:{x['id']}:{did}:{p}")] for x in subset]; nav=[]
    if p: nav.append(b("⬅️ Предыдущая",f"book:disc:{did}:{p-1}"))
    if p*7+7<len(ts): nav.append(b("➡️ Следующая",f"book:disc:{did}:{p+1}"))
    if nav: rows.append(nav)
...
async def edit_field(c:CallbackQuery,state:FSMContext):
    field=c.data.split(':')[1];await state.update_data(field=field);await state.set_state(HWEdit.value);await c.answer();await show(c.bot,c.from_user.id,"Введите новое значение. Для готового ответа можно отправить текст или документ.")
...
async def asch_date(c,state):
    ...
    if day:
        return await edit_or_answer(c,f"📅 <b>{ru_date(d)}</b>\nРасписание уже опубликовано.",ik(nav+[[b("✏️ Редактировать",f"asch:edit:{d.isoformat()}")],[b("🗑 Удалить",f"asch:del:{d.isoformat()}")],[b("🗓 Календарь месяца",f"asch:month:{d.year}:{d.month}")],[b("🏠 Админ-панель","adm:back")]]))
...
async def schc_count(c,state): await state.update_data(count=int(c.data.split(':')[-1]),idx=1);await state.set_state(ScheduleCreate.lesson_start);await c.answer();await edit_or_answer(c,"Выберите время 1-й пары:",ik([[b(f"{s}–{e}",f"schc:time:{i}")] for i,(s,e) in enumerate(SLOT_TIMES,1)]))
...
async def asche_time(c:CallbackQuery,state:FSMContext):
    ...
    await edit_or_answer(c,"🕐 <b>Выберите новое время пары:</b>",ik([[b(f"{s}–{e}",f"asche:settime:{i}")] for i,(s,e) in enumerate(SLOT_TIMES,1)]+[[b("🔙 Назад",f"asche:lesson:{d}:{lid}")]]))
...
async def asche_add(c:CallbackQuery,state:FSMContext):
    ...
    await edit_or_answer(c,"➕ <b>Выберите время новой пары:</b>",ik([[b(f"{s}–{e}",f"asche:addtime:{i}")] for i,(s,e) in enumerate(SLOT_TIMES,1)]+[[b("🔙 Назад",f"asch:edit:{d}")]]))
...
# Broadcast
async def broadcast_start(c,state): await state.set_state(Broadcast.text);await c.answer();await show(c.bot,c.from_user.id,"📢 Введите текст уведомления всем:")
