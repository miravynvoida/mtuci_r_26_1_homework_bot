...
async def admin_hws(c: CallbackQuery):
    if not is_admin(c.from_user.id):
        await c.answer("Нет доступа.", show_alert=True)
        return
    # Сразу закрываем индикатор загрузки Telegram, чтобы кнопка не выглядела
    # как будто вообще не нажалась, даже если при построении списка возникнет ошибка.
    await c.answer()
    try:
        await hw_list_page(c, False, 0)
    except Exception:
        log.exception("Не удалось открыть раздел 'Управление ДЗ'")
        try:
            await c.bot.send_message(
                c.message.chat.id,
                "❌ Не удалось открыть раздел «Управление ДЗ». Проверь лог бота на хостинге.",
                reply_markup=kb([[InlineKeyboardButton(text="⬅️ Админ-панель", callback_data="adm")]]),
            )
        except Exception:
            pass
