...
async def archive_expired_homework():
    """Переносит ДЗ с прошедшей датой сдачи в архив."""
    conn = await db()
    await conn.execute("""
        UPDATE homework SET hidden=1
        WHERE hidden=0
          AND date(substr(due_date,7,4)||"-"||substr(due_date,4,2)||"-"||substr(due_date,1,2)) < date("now")
    """)
    await conn.commit()
    await conn.close()
...
async def disciplines_keyboard():
    await archive_expired_homework()
    conn = await db()
...
async def discipline(c: CallbackQuery):
    ...
    cur = await conn.execute(
        ...
        WHERE discipline_id=? AND hidden=0
          AND date(substr(due_date,7,4)||"-"||substr(due_date,4,2)||"-"||substr(due_date,1,2)) >= date("now")
        ORDER BY date(substr(due_date,7,4)||"-"||substr(due_date,4,2)||"-"||substr(due_date,1,2)) ASC, id DESC
        """,
        (did,)
    )
...
async def homework_view(c: CallbackQuery):
    ...
    if not h:
        ...
        return
    if h["hidden"] and not is_admin(c.from_user.id):
        await c.answer("Это ДЗ уже находится в архиве.", show_alert=True)
        return
    text = (
...
async def admin_hw_list(c: CallbackQuery, page: int = 0, archive: bool = False):
    if not is_admin(c.from_user.id):
        return
    await archive_expired_homework()
    conn = await db()
    where = "h.hidden=1" if archive else "h.hidden=0"
    cur = await conn.execute(f"""
        SELECT h.id, h.published_date, h.due_date, d.name, d.emoji
        FROM homework h JOIN disciplines d ON d.id=h.discipline_id
        WHERE {where}
        ORDER BY date(substr(h.published_date,7,4)||'-'||substr(h.published_date,4,2)||'-'||substr(h.published_date,1,2)) ASC, h.id ASC
    """)
    all_rows = await cur.fetchall()
    await conn.close()
    total_pages = max(1, (len(all_rows) + HW_PAGE_SIZE - 1) // HW_PAGE_SIZE)
    page = max(0, min(page, total_pages - 1))
    page_rows = all_rows[page * HW_PAGE_SIZE:(page + 1) * HW_PAGE_SIZE]

    buttons = []
    for r in page_rows:
        # Короткая кнопка: подробности открываются после нажатия.
        buttons.append(InlineKeyboardButton(
            text=f'{r["emoji"]} {r["name"]} • {r["due_date"]}',
            callback_data=f'adhw:{r["id"]}'
        ))
    rows = [buttons[i:i+2] for i in range(0, len(buttons), 2)]
    nav = []
    if page > 0:
        nav.append(InlineKeyboardButton(text="⬅️ Предыдущая", callback_data=f'admhwpage:{int(archive)}:{page-1}'))
    if page < total_pages - 1:
        nav.append(InlineKeyboardButton(text="Следующая ➡️", callback_data=f'admhwpage:{int(archive)}:{page+1}'))
    if nav:
        rows.append(nav)
    rows.append([InlineKeyboardButton(text="⬅️ Управление ДЗ", callback_data="adm:hws")])
    title = "🗄 <b>Архив ДЗ</b>" if archive else "📋 <b>Все активные ДЗ</b>"
    if all_rows:
        text = f"{title}\n\nСтраница {page+1} из {total_pages}. Старые ДЗ находятся в начале списка."
    else:
        text = f"{title}\n\nЗдесь пока нет ДЗ."
    await replace_callback(c, text, kb(rows))
...
async def admin_hws(c: CallbackQuery):
    if not is_admin(c.from_user.id): return
    await replace_callback(c, "📋 <b>Управление ДЗ</b>\n\nВыбери раздел:", kb([
        [InlineKeyboardButton(text="📋 Все ДЗ", callback_data="admhwpage:0:0"),
         InlineKeyboardButton(text="🗄 Архив", callback_data="admhwpage:1:0")],
        [InlineKeyboardButton(text="⬅️ Админ-панель", callback_data="adm")]
    ]))

@router.callback_query(F.data.startswith("admhwpage:"))
async def admin_hw_page(c: CallbackQuery):
    if not is_admin(c.from_user.id): return
    _, archive, page = c.data.split(":")
    await admin_hw_list(c, int(page), bool(int(archive)))
...
async def admin_hw_detail(c: CallbackQuery):
    ...
    await archive_expired_homework()
    conn = await db()
    cur = await conn.execute("SELECT h.*, d.name, d.emoji FROM homework h JOIN disciplines d ON d.id=h.discipline_id WHERE h.id=?", (hid,))
    h = await cur.fetchone()
    cur = await conn.execute("SELECT COUNT(*) AS cnt FROM homework_media WHERE homework_id=?", (hid,))
    media_count = (await cur.fetchone())["cnt"]
    await conn.close()
    if not h:
        await c.answer("ДЗ не найдено.", show_alert=True); return
    status = "🗄 В архиве" if h["hidden"] else "🟢 Активно"
    ...
    if h["explanation"]:
        text += f'\n\n💬 <b>Пояснение:</b>\n{esc(h["explanation"])}'
    hidden_int = int(h["hidden"])
    action = []
    if h["hidden"]:
        action.append(InlineKeyboardButton(text="♻️ Вернуть из архива", callback_data=f"restorehw:{hid}"))
    else:
        action.append(InlineKeyboardButton(text="🗄 Скрыть в архив", callback_data=f"archivehw:{hid}"))
    rows = [action,
            [InlineKeyboardButton(text="✏️ Редактировать", callback_data=f"edhw:{hid}"),
             InlineKeyboardButton(text="🗑 Удалить", callback_data=f"delhwq:{hid}")],
            [InlineKeyboardButton(text="⬅️ Назад", callback_data=f"admhwpage:{hidden_int}:0")]]
    await replace_callback(c, text, kb(rows))

@router.callback_query(F.data.startswith("archivehw:"))
async def archive_hw(c: CallbackQuery):
    if not is_admin(c.from_user.id): return
    hid = int(c.data.split(":")[1])
    conn = await db(); await conn.execute("UPDATE homework SET hidden=1 WHERE id=?", (hid,)); await conn.commit(); await conn.close()
    await admin_hw_list(c, 0, True)

@router.callback_query(F.data.startswith("restorehw:"))
async def restore_hw(c: CallbackQuery):
    if not is_admin(c.from_user.id): return
    hid = int(c.data.split(":")[1])
    conn = await db(); cur = await conn.execute("SELECT due_date FROM homework WHERE id=?", (hid,)); h = await cur.fetchone()
    if not h:
        await conn.close(); await c.answer("ДЗ не найдено.", show_alert=True); return
    # Возвращаем только если срок не истёк. При необходимости дату можно изменить через «Редактировать».
    due = valid_date(h["due_date"])
    if not due or due < date.today():
        await conn.close()
        await replace_callback(c, "⚠️ Срок сдачи уже прошёл. Сначала измени дату сдачи, затем верни ДЗ из архива.", kb([
            [InlineKeyboardButton(text="✏️ Редактировать", callback_data=f"edhw:{hid}")],
            [InlineKeyboardButton(text="⬅️ Архив", callback_data="admhwpage:1:0")]
        ]))
        return
    await conn.execute("UPDATE homework SET hidden=0 WHERE id=?", (hid,)); await conn.commit(); await conn.close()
    await admin_hw_list(c, 0, False)
...
async def admin_hw_delete_confirm(c: CallbackQuery):
    ...
    if not h: await c.answer("Не найдено.", show_alert=True); return
    await replace_callback(c, f'🗑 <b>Удалить ДЗ окончательно?</b>\n\n{h["emoji"]} {esc(h["name"])}\n{esc(h["text"])}', kb([
        [InlineKeyboardButton(text="🗑 Да, удалить", callback_data=f"delhw:{hid}"), InlineKeyboardButton(text="❌ Отмена", callback_data=f"adhw:{hid}")]
    ]))
...
async def admin_hw_delete(c: CallbackQuery):
    if not is_admin(c.from_user.id): return
    hid = int(c.data.split(":")[1])
    conn = await db(); await conn.execute("DELETE FROM homework WHERE id=?", (hid,)); await conn.commit(); await conn.close()
    await admin_hws(c)

@router.callback_query(F.data.startswith("edhw:"))
async def edit_hw_menu(c: CallbackQuery):
    if not is_admin(c.from_user.id): return
    hid = int(c.data.split(":")[1])
    await replace_callback(c, "✏️ <b>Редактирование ДЗ</b>\n\nЧто изменить?", kb([
        [InlineKeyboardButton(text="📝 Задание", callback_data=f"edhwtext:{hid}"), InlineKeyboardButton(text="📅 Дата публикации", callback_data=f"edhwpub:{hid}")],
        [InlineKeyboardButton(text="⏳ Дата сдачи", callback_data=f"edhwdue:{hid}"), InlineKeyboardButton(text="💬 Пояснение", callback_data=f"edhwexp:{hid}")],
        [InlineKeyboardButton(text="📎 Файлы", callback_data=f"edhwmedia:{hid}")],
        [InlineKeyboardButton(text="⬅️ Назад к ДЗ", callback_data=f"adhw:{hid}")]
    ]))

@router.callback_query(F.data.startswith("edhwtext:"))
async def edit_hw_text_start(c: CallbackQuery, state: FSMContext):
    hid = int(c.data.split(":")[1]); await state.clear(); await state.update_data(hid=hid); await state.set_state(EditHW.text)
    await replace_callback(c, "📝 Введи новый текст задания:", kb([[InlineKeyboardButton(text="❌ Отмена", callback_data=f"edhw:{hid}")]]))

@router.message(EditHW.text)
async def edit_hw_text(message: Message, state: FSMContext):
    data = await state.get_data(); text_value = (message.text or "").strip()
    if not text_value: await message.answer("❌ Текст не может быть пустым."); return
    conn = await db(); await conn.execute("UPDATE homework SET text=? WHERE id=?", (text_value, data["hid"])); await conn.commit(); await conn.close(); await state.clear()
    await replace_message(message, "✅ Задание изменено.", kb([[InlineKeyboardButton(text="⬅️ К ДЗ", callback_data=f"adhw:{data['hid']}")]]))

async def edit_hw_date_start(c: CallbackQuery, state: FSMContext, field: str, prompt: str):
    hid = int(c.data.split(":")[1]); await state.clear(); await state.update_data(hid=hid, field=field); await state.set_state(EditHW.published if field == "published_date" else EditHW.due)
    await replace_callback(c, prompt, kb([[InlineKeyboardButton(text="❌ Отмена", callback_data=f"edhw:{hid}")]]))

@router.callback_query(F.data.startswith("edhwpub:"))
async def edit_hw_pub_start(c: CallbackQuery, state: FSMContext): await edit_hw_date_start(c, state, "published_date", "📅 Введи новую дату публикации в формате <b>ДД.ММ.ГГГГ</b>:")

@router.message(EditHW.published)
async def edit_hw_pub(message: Message, state: FSMContext):
    d = valid_date(message.text or ""); data = await state.get_data()
    if not d: await message.answer("❌ Неверная дата."); return
    conn = await db(); await conn.execute("UPDATE homework SET published_date=? WHERE id=?", (d.strftime("%d.%m.%Y"), data["hid"])); await conn.commit(); await conn.close(); await state.clear()
    await replace_message(message, "✅ Дата публикации изменена.", kb([[InlineKeyboardButton(text="⬅️ К ДЗ", callback_data=f"adhw:{data['hid']}")]]))

@router.callback_query(F.data.startswith("edhwdue:"))
async def edit_hw_due_start(c: CallbackQuery, state: FSMContext): await edit_hw_date_start(c, state, "due_date", "⏳ Введи новую дату сдачи в формате <b>ДД.ММ.ГГГГ</b>:")

@router.message(EditHW.due)
async def edit_hw_due(message: Message, state: FSMContext):
    d = valid_date(message.text or ""); data = await state.get_data()
    if not d: await message.answer("❌ Неверная дата."); return
    conn = await db(); await conn.execute("UPDATE homework SET due_date=? WHERE id=?", (d.strftime("%d.%m.%Y"), data["hid"])); await conn.commit(); await conn.close(); await state.clear()
    await replace_message(message, "✅ Дата сдачи изменена.", kb([[InlineKeyboardButton(text="⬅️ К ДЗ", callback_data=f"adhw:{data['hid']}")]]))

@router.callback_query(F.data.startswith("edhwexp:"))
async def edit_hw_exp_start(c: CallbackQuery, state: FSMContext):
    hid = int(c.data.split(":")[1]); await state.clear(); await state.update_data(hid=hid); await state.set_state(EditHW.explanation)
    await replace_callback(c, "💬 Введи новое пояснение или отправь <b>—</b>, чтобы убрать его:", kb([[InlineKeyboardButton(text="❌ Отмена", callback_data=f"edhw:{hid}")]]))

@router.message(EditHW.explanation)
async def edit_hw_exp(message: Message, state: FSMContext):
    data = await state.get_data(); value = (message.text or "").strip(); value = "" if value in {"", "—", "-"} else value
    conn = await db(); await conn.execute("UPDATE homework SET explanation=? WHERE id=?", (value, data["hid"])); await conn.commit(); await conn.close(); await state.clear()
    await replace_message(message, "✅ Пояснение изменено.", kb([[InlineKeyboardButton(text="⬅️ К ДЗ", callback_data=f"adhw:{data['hid']}")]]))

@router.callback_query(F.data.startswith("edhwmedia:"))
async def edit_hw_media(c: CallbackQuery, state: FSMContext):
    hid = int(c.data.split(":")[1]); conn = await db(); cur = await conn.execute("SELECT * FROM homework_media WHERE homework_id=? ORDER BY id", (hid,)); ms = await cur.fetchall(); await conn.close()
    rows = [[InlineKeyboardButton(text=f'🗑 Удалить файл {i+1}', callback_data=f"edhwdelmedia:{m['id']}:{hid}")] for i,m in enumerate(ms)]
    rows.append([InlineKeyboardButton(text="➕ Добавить файл", callback_data=f"edhwaddmedia:{hid}")])
    rows.append([InlineKeyboardButton(text="⬅️ Назад", callback_data=f"edhw:{hid}")])
    await state.clear(); await replace_callback(c, "📎 <b>Файлы ДЗ</b>\n\nНажми на файл, чтобы удалить его, или добавь новый.", kb(rows))

@router.callback_query(F.data.startswith("edhwdelmedia:"))
async def edit_hw_delete_media(c: CallbackQuery):
    if not is_admin(c.from_user.id): return
    _, mid, hid = c.data.split(":")
    conn = await db(); await conn.execute("DELETE FROM homework_media WHERE id=?", (int(mid),)); await conn.commit(); await conn.close()
    # Обновляем экран файлов без смены состояния.
    ...
    rows = [[InlineKeyboardButton(text=f'🗑 Удалить файл {i+1}', callback_data=f"edhwdelmedia:{m['id']}:{hid}")] for i,m in enumerate(ms)]
    rows.append([InlineKeyboardButton(text="➕ Добавить файл", callback_data=f"edhwaddmedia:{hid}")]); rows.append([InlineKeyboardButton(text="⬅️ Назад", callback_data=f"edhw:{hid}")])
    await replace_callback(c, "✅ Файл удалён.\n\n📎 <b>Файлы ДЗ</b>", kb(rows))

@router.callback_query(F.data.startswith("edhwaddmedia:"))
async def edit_hw_add_media_start(c: CallbackQuery, state: FSMContext):
    hid = int(c.data.split(":")[1]); await state.clear(); await state.update_data(hid=hid); await state.set_state(EditHW.media)
    await replace_callback(c, "📎 Отправляй файлы, фото или видео.\n\nКогда закончишь — нажми «Готово».", kb([[InlineKeyboardButton(text="✅ Готово", callback_data=f"edhwmedia_done:{hid}")], [InlineKeyboardButton(text="❌ Отмена", callback_data=f"edhw:{hid}")]]))

@router.message(EditHW.media)
async def edit_hw_media_message(message: Message, state: FSMContext):
    data = await state.get_data(); item = None
    if message.document: item = ("document", message.document.file_id, message.caption)
    elif message.photo: item = ("photo", message.photo[-1].file_id, message.caption)
    elif message.video: item = ("video", message.video.file_id, message.caption)
    if not item: await message.answer("Пришли файл, фото или видео."); return
    conn = await db(); await conn.execute("INSERT INTO homework_media(homework_id,kind,file_id,caption) VALUES(?,?,?,?)", (data["hid"], item[0], item[1], item[2])); await conn.commit(); await conn.close()
    await message.answer("✅ Файл добавлен.")

@router.callback_query(EditHW.media, F.data.startswith("edhwmedia_done:"))
async def edit_hw_media_done(c: CallbackQuery, state: FSMContext):
    hid = int(c.data.split(":")[1]); await state.clear(); await edit_hw_menu(c)
...
async def admin_books(c: CallbackQuery):
    ...
    buttons = []
    for d in ds:
        status = "🟢" if d["textbooks_enabled"] else "⚪"
        buttons.append(InlineKeyboardButton(
...
async def admbooks(c: CallbackQuery):
    ...
    if not d:
        await c.answer("Дисциплина не найдена.", show_alert=True)
        return
    book_buttons = [InlineKeyboardButton(text=f'📕 {b["title"]}', callback_data=f"delbookq:{b['id']}") for b in bs]
    rows = [book_buttons[i:i+2] for i in range(0, len(book_buttons), 2)]
